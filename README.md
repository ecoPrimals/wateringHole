...
| **sweetGrass** | Attribution | Semantic provenance (v0.7.27). W3C PROV-O braids, fair attribution, 32 JSON-RPC methods + tarpc 0.37 + REST + UDS, UniBin, ecoBin, 1,448 tests + 56 Docker CI, 91.7% coverage, Edition 2024, BTSP Phase 2 first-line auto-detect + crypto relay aligned with `beardog_types`, BraidBackend enum dispatch (zero async-trait/dyn), sled eliminated, shared ecosystem path constants, `detect_protocol` three-way multiplexer, pedantic+nursery clean | Production |
