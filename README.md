...
| **ToadStool** | Hardware Infrastructure | Hardware discovery, capability probing, compute orchestration: CPU, GPU, NPU, WASM, containers, edge. 23,000+ tests (9,206+ lib), **112 JSON-RPC methods** (17 groups). Node Atomic for sovereign compute. ecoBin v3.0 certified. riboCipher CLEAR + MitoBeacon. Zero-copy dispatch. Cross-architecture (`cargo check --target x86_64-pc-windows-gnu` passes, S329). Zero clippy warnings (S330). Borrowed deserialization sweep — zero `from_value(v.clone())` in handlers (S331). Zero `/tmp` hardcoding (S328). 100% env centralized. VFIO sovereign dispatch validated. | Production (A++ GOLD, S331+) |
