...
| **sweetGrass** | Attribution | Semantic provenance (v0.7.27). W3C PROV-O braids, fair attribution, 32 JSON-RPC methods + tarpc 0.37 + REST + UDS, UniBin, ecoBin, 1,436 tests + 56 Docker CI, 91.7% coverage, Edition 2024, BTSP Phase 2 auto-detect, BraidBackend enum dispatch (zero async-trait/dyn), sled eliminated, shared ecosystem path constants, PeekedStream protocol multiplexer, pedantic+nursery clean | Production |
