...
| **biomeOS** | Orchestration | Composition primal: Neural API (320+ translations, 27 domains), 5 coordination patterns (Sequential, Parallel, ConditionalDag, Pipeline streaming, Continuous 60Hz), capability routing, NUCLEUS composition, PathwayLearner optimization, NDJSON streaming, bonding model, Dark Forest coordination, provenance trio wiring, BTSP Phase 3 FULL, cellular deploy via neural-api, tick loop event relay, graph signing (BLAKE3+Ed25519), coordination key caching, capability-based identity | Production (v3.39, 8,076+ tests, zero C deps, Security A++ LEGENDARY) |
...
| **sweetGrass** | Attribution | Semantic provenance (v0.7.29). W3C PROV-O braids, fair attribution, 32 JSON-RPC methods + tarpc 0.37 + REST + UDS, UniBin, ecoBin, 1,482 tests + 56 Docker CI, 91.7% coverage, Edition 2024, BTSP Phase 3 (`btsp.negotiate` + ChaCha20-Poly1305 AEAD + HKDF-SHA256 session keys), BearDog `crypto.sign` delegation, BraidBackend enum dispatch (zero async-trait/dyn), `detect_protocol` three-way multiplexer, pedantic+nursery clean | Production |
...
- `fossilRecord/UPSTREAM_GAP_STATUS_APR20_2026.md` — Upstream gap tracker (biomeOS v3.37 absorbed, May 2)
...
### Handoffs
- `handoffs/*.md` — 15 active session handoffs (April 28 – May 2, 2026)
- `handoffs/archive/` — Fossil record (743+ archived handoffs)
