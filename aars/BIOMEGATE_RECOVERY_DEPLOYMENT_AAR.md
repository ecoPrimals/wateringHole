...
**Installed Aug 2**: Titan V (GV100, SM70, 12GB HBM2) + Tesla K80 (GK210×2, SM37,
2×12GB GDDR5). See `BIOMEGATE_GPU_CRANKSHAFT_AAR.md` for VFIO binding and revalidation.
...
|---|---------|-------|--------|
| ~~1~~ | ~~SSH key not registered on Forgejo~~ | ~~biomeGate~~ | eastGate documented key. Forgejo registration disabled (admin-only). |
| ~~2~~ | ~~WG IP not assigned~~ | ~~eastGate overwatch~~ | **REGISTERED** — biomeGate in ecosystem_manifest.toml. |
...
- Switched nvidia-driver-595-open → nvidia-driver-595 (proprietary) for Titan V
  (Volta SM70) host driver support. K80 (Kepler SM37) bypassed via VFIO.
- sudo NOPASSWD configured for `biomegate` user (all IDE terminals have root).

---

## Superseded By

- **`BIOMEGATE_GPU_CRANKSHAFT_AAR.md`** — GPU fleet VFIO binding, revalidation matrix
- **`springs/hotSpring/specs/BIOMEGATE_REVALIDATION_SPEC.md`** — Diesel engine + hotQCD goals

---

*biomeGate RECOVERED → GPU CRANKSHAFT LIVE. 3 sovereign GPUs on VFIO. K80 unretired.
Diesel engine + hotQCD revalidation in progress.*
