...
- **Python runtime**: pyo3 FFI removed per ecoBin v3.0. Python dispatched as `type = "native"` with explicit interpreter path (`/home/irongate/miniforge3/envs/bioinfo/bin/python3`).
...
## Provenance Pipeline Elevation (2026-05-04)

Following the Node Atomic validation, the composition was elevated to **Nest Atomic + ToadStool (9 primals)** by deploying NestGate, rhizoCrypt, loamSpine, and sweetGrass alongside the running Node Atomic.

### Additional Primals

| Primal | Port | Protocol | Status |
|--------|------|----------|--------|
| nestgate | 9500 | Newline-delimited JSON-RPC (TCP) | OK |
| rhizocrypt | 9601 | Newline-delimited JSON-RPC (TCP) | OK |
| loamspine | 9700 | HTTP JSON-RPC | OK |
| sweetgrass | 39085 | HTTP JSON-RPC | OK |

### Expanded Workload Suite (10 workloads, 235+ checks)

| Workload | Checks | Status | Data |
|----------|--------|--------|------|
| R Industry Parity | 53/53 | PASS | R gold-standard vectors |
| Fajgenbaum Pathway | 8/8 | PASS | JCI 2019 proteomic data |
| Diversity Indices | 27/27 | PASS | Synthetic + QIIME2 baselines |
| Gonzales CPU Parity | 43/43 | PASS | Analytical + deterministic seeds |
| Algae 16S | 34/34 | PASS | **Real NCBI: SRR7760408 (11.9M reads)** |
| 16S Pipeline | 37/37 | PASS | Synthetic vectors |
| Cold Seep Pipeline | 8/8 | PASS | Synthetic (Ruff et al.) |
| Real NCBI Pipeline | 25/25 | PASS | Synthetic fallback |
| Python 16S Baseline | — | RUN | **Real NCBI: SRR7760408 (50K reads)** |
| Python Benchmark | — | RUN | Generated benchmark vectors |

### Provenance Chain

Every artifact and pipeline step was wrapped in provenance via `provenance_pipeline.sh`:

- **4 NCBI FASTQ files** content-addressed with BLAKE3, stored in NestGate
- **10 workload executions** bracketed by rhizoCrypt DAG events (ExperimentStart + Result)
- **24 total DAG events** in session `019df42d-0fba-7170-a216-2f3b282e3fb9`
- **Merkle root**: `292ebbcf8f02561aaa6c67b532ebbefc14c32192cf3dfb733ce81e45fba50f9e`
- **loamSpine commit**: Spine `019df41b-40c5-7b93-bf35-79dbd95a2cb3`, index 55 (SessionCommit)
- **sweetGrass braid**: `urn:braid:292ebbcf...` with ed25519 witness by `did:key:z6MkQOXw_...`

### New Gaps Discovered

- toadStool does not expand environment variables (`${VAR}`) in workload TOMLs — absolute paths required
- loamSpine `data_hash`/`merkle_root` fields require `[u8; 32]` byte arrays, not hex strings
- sweetGrass TCP port requires BTSP handshake — HTTP endpoint used instead
- Python workloads don't produce `[OK]`/`[FAIL]` markers — check counting requires separate heuristics

See `COMPOSITION_GAPS.md` for updated gap analysis (Gaps 8-10 added).
