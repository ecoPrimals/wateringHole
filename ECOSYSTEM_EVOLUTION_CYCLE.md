...
| **RESOLVED** | Songbird | 0 `#[async_trait]`, 0 finite `dyn`, 0 production mocks, 0 bare `#[allow]`, 0 blanket lint suppress, 0 hardcoded IPs/ports, 0 unused deps, 0 `Box<dyn Error>` in production, 0 files >800L. W182: BTSP Phase 3 spec alignment (primalSpring audit — `bond_type` + `preferred_cipher` + cipher floor enforcement + 12B nonce). W181: port canonicalization (8 new constants, `discovery_port()` bug fix, tarpc harmonized, 5 IPs evolved, mDNS dedup, serde alias evolved). W180: BTSP Phase 3 `btsp.negotiate` server-side (ChaCha20-Poly1305 encrypted framing, HKDF session keys, NULL fallback). W179: coverage +92 tests. W178: deep debt (anyhow). W177: signed IPC. W176: smart refactor. Zero `::new()` in production — 7,803 lib tests, 0 failures |
