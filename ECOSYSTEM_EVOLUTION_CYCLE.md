...
- **Stadial gate cleared**: BearDog W56, Songbird W158, NestGate 43w, ToadStool S203t, petalTongue v1.6.7, sweetGrass stadial — zero async-trait, zero finite dyn, Edition 2024
...
| **RESOLVED** | Songbird | 0 `#[async_trait]`, 0 finite `dyn`, 0 production mocks, 0 bare `#[allow]`, 0 blanket lint suppress, 0 hardcoded IPs/ports (W157), 0 unused deps, 0 `Box<dyn Error>` in production. Startup resilience (W156), ~12.4k lines dead test code removed (W157), BTSP relay wire-protocol aligned with BearDog's `btsp.server.*` methods (W158: 6 param/response mismatches fixed, `ServerHello` session_id added, BearDog challenge sourcing corrected) — Wave 158, 7,387 tests, 0 failures |
