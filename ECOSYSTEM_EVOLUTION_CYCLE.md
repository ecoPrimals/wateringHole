...
| **RESOLVED** | Songbird | 0 `#[async_trait]`, 0 finite `dyn`, 0 production mocks, 0 bare `#[allow]`, 0 blanket lint suppress, 0 hardcoded IPs/ports, 0 unused deps, 0 `Box<dyn Error>` in production, 0 files >800L. W177: signed IPC registrations (Ed25519 via BearDog delegation, canonical payload, graceful standalone). W176: deep debt (smart refactor 1121L→directory module, remaining hardcoded fallbacks → constants, `Result<_,String>` → `anyhow::Result`, deprecated `DEFAULT_BIND_ADDRESS`, centralized socket paths). W175: PG-51 verified, ENVIRONMENT_VARIABLES.md. W174: hardcoded IP/port, flaky tests, dep cleanup, +18 tests. W173: PG-51 fix. W172: doc reconciliation. W171: coverage 71.28%→73.41% (+271 tests). W170: CLI flag alignment. W169: `new_direct()`. W168: routing+seed. W167: error frames. W166: doc reconciliation. W165: dep cleanup. Zero `::new()` in production — 7,692 lib tests, 0 failures |
