...
- **primalSpring v0.9.16**: guideStone Level 4 (67/67 live NUCLEUS, 41/41 bare, BLAKE3 P3), fragment-first graphs, 601 tests, plasmidBin depot documented, deep debt evolution (capability-based discovery, zero hardcoded primal names, all files under 800 LOC)
...
- **plasmidBin genomeBin depot**: Full cross-architecture binary depot (42 binaries, 6 target triples). `build_ecosystem_genomeBin.sh` replaces musl-only script with 9-target matrix (Tier 1 MUST / Tier 2 SHOULD / Tier 3 NICE per ecoBin Architecture Standard). Springs pull, deploy NUCLEUS, validate externally. See `primalSpring/wateringHole/PLASMINBIN_DEPOT_PATTERN.md`.
