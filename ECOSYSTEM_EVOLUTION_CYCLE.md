...
- **Stadial gate cleared**: BearDog W56, Songbird W187, NestGate 43w, ToadStool S203t, petalTongue v1.6.7, sweetGrass stadial — zero async-trait, zero finite dyn, Edition 2024
...
| **RESOLVED** | Songbird | 0 `#[async_trait]`, 0 finite `dyn`, 0 production mocks, 0 bare `#[allow]`, 0 blanket lint suppress, 0 hardcoded IPs/ports, 0 unused deps, 0 `Box<dyn Error>` in production, 0 files >800L, 0 unsafe. W187: smart refactor (`connection.rs` 1043→694L via `#[path]` test extraction), 15 "BearDog" error strings evolved to "security provider", 4 new timeout constants. W186: BTSP Phase 3 live connection verification (4 tests — encrypted frame loop on live duplex, negotiate dispatch, null fallback, mock-provider E2E). W185: deep debt (11 new timeout constants, JSON-RPC constructors, 15+ production literals centralized, primal codenames evolved). W184: BTSP Phase 3 dispatch fix — `btsp.negotiate` wired into binary-framed BTSP path (`handle_btsp_frames`), all 3 transport paths now covered. W183: deep debt (lint evolution, 8 timeout constants, hardcoded elimination, dep refresh). W182: BTSP Phase 3 spec alignment (primalSpring audit — `bond_type` + `preferred_cipher` + cipher floor enforcement + 12B nonce). W181: port canonicalization (8 new constants, `discovery_port()` bug fix, tarpc harmonized, 5 IPs evolved, mDNS dedup, serde alias evolved). W180: BTSP Phase 3 `btsp.negotiate` server-side (ChaCha20-Poly1305 encrypted framing, HKDF session keys, NULL fallback). W179: coverage +92 tests. W178: deep debt (anyhow). W177: signed IPC. W176: smart refactor. Zero `::new()` in production — 7,803 lib tests, 0 failures |
