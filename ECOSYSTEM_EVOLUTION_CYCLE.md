...
| **RESOLVED** | Songbird | 0 `#[async_trait]`, 0 finite `dyn`, 0 production mocks, 0 bare `#[allow]`, 0 blanket lint suppress, 0 hardcoded IPs/ports, 0 unused deps, 0 `Box<dyn Error>` in production. W174: deep debt (hardcoded IP/port → constants, flaky test → ScopedEnv, removed `mdns`/`once_cell`, unified `tempfile`, +18 tests). W173: PG-51 fix (family-scoped socket discovery). W172: doc reconciliation. W171: coverage 71.28%→73.41% (+271 tests). W170: CLI flag alignment. W169: `new()` → `new_direct()`. W168: routing + seed. W167: error frames. W166: doc reconciliation. W165: dep cleanup. Zero `::new()` in production — 497 lib tests, 0 failures |
