...
- **V2.3 (April 2, 2026)**: primalSpring Phase 23i re-audit. Overstep scan confirms all documented boundaries — no new violations. rhizoCrypt/loamSpine TCP listeners confirmed standard IPC dual-mode (not networking overstep). Key compliance evolution: Songbird 395→8 clippy warnings; loamSpine fmt now passes; NestGate regressed (test compile errors) but gained `deny.toml`; petalTongue/toadStool tests now pass. All 8 open primalSpring gaps reclassified to Low (NG-01 Medium→Low).
