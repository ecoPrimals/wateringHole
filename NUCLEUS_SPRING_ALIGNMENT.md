...
| **neuralSpring** | V153 | **5** | 888 (IPC-first) | **Node** + Meta | `downstream_manifest.toml` | **Active** — S202b: CapabilityRouter IPC, NestGate wired, `--format json`, Thread 5, 7 IPC modules |
...
| **neuralSpring** | **5** | optional `barracuda` (IPC-first, `default = []`) | 0% direct | `src/ipc/` tree (7 modules, CapabilityRouter) | Wired (deploy graphs) | Wired (composition.status) |
...
6. **neuralSpring** (5) — `src/ipc/` tree complete (7 modules), CapabilityRouter discovery, NestGate content-addressed storage wired
