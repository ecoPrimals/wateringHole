...
| **hotSpring** | **guideStone Level 5 CERTIFIED** (reference implementation): all 5 properties, `hotspring_guidestone` binary (BLAKE3 P3 via `primalspring::checksums`, protocol tolerance via `is_protocol_error()`, family-aware discovery), `validate_primal_proof` (9 probes, 10 capabilities), `validation/` artifact (59/59 × 5 substrates), deploy graph in `graphs/`. Absorbed primalSpring v0.9.16. Bare mode verified: 14/14 PASS (4 SKIP). `scripts/validate-primal-proof.sh` wraps bare + NUCLEUS deployment workflow. |
