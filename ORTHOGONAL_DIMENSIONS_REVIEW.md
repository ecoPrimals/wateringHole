...
| G56/G67 | **Neural API activation & Stage 2 routing** | **ACTIVE — N2-N5 VERIFIED (87/91), RIBOCIPHER AUTO-DETECT SHIPPED** | biomeOS on all 6 NUCLEUS gates. 578 tests. **N-series verified by primalSpring**: exp118 14/14, exp119 12/12, exp120 29/29, exp121 32/36. Mean dispatch: **1.3ms** (was 15s). **9/11 primals forward**. **riboCipher auto-detect shipped** (`1ff5859c`): domain-level `ribocipher: bool` in TOML, sweetGrass/rhizoCrypt auto-route through `send_ribocipher_jsonrpc()`. westGate: 990,500 inline-braided, 26 caps resolve. **swarmVine Phase 2 + songBird seam**: wired. Remaining: **rebuild + redeploy** biomeOS with dispatch fix + riboCipher auto-detect (depot binary is Jul 15 pre-fix), toadStool TARPC shim (architecture decision), fleet-wide swarmVine deploy, biomeOS → swarmVine gossip table. *(Merged G56+G67.)* |
...
**Summary**: Wave 157a Ecosystem Checkpoint — **N2-N5 VERIFIED (87/91). RIBOCIPHER AUTO-DETECT SHIPPED. 990K BRAIDED. SWARMVINE WIRED.** 17 COMPLETE, 21 ACTIVE, 26 GLACIAL (64 total). primalSpring verified dispatch fix: exp118-121, 1.3ms mean dispatch (was 15s), 9/11 primals forward. biomeOS shipped riboCipher auto-detect (`1ff5859c`): domain-level TOML flag, sweetGrass/rhizoCrypt auto-route. cellMembrane transport unification: `#[cfg(unix)]` 7→3, 1,329 tests. **Depot rebuild needed** — current binary is Jul 15 pre-fix. **16 primals. ZERO P0/P1. ~141K+ tests.**

**Phase shift**: **"Neural API N2-N5 verified + riboCipher auto-detect shipped + depot rebuild is the gate to fleet-wide activation."** primalSpring verified biomeOS dispatch fix on eastGate: 87/91 tests, 1.3ms mean dispatch (was 15s timeout), 9/11 primals forward. biomeOS shipped riboCipher auto-detect (`1ff5859c`): TOML domain-level `ribocipher: bool`, sweetGrass + rhizoCrypt auto-route through riboCipher pool. **Critical path**: depot rebuild (current binary is Jul 15 pre-fix) → gate redeploy → sweetGrass forwarding (32/36→36/36) → fleet-wide Neural API activation. toadStool TARPC mismatch is architecture, not P1. swarmVine Phase 2 wired + songBird seam LIVE. cellMembrane transport unification shipped. Next: (1) **depot rebuild + gate redeploy** with dispatch fix + riboCipher auto-detect, (2) fleet-wide swarmVine deployment + biomeOS → gossip table, (3) inner membrane: bearDog riboCipher Tier 2, songBird `0xED` acceptance, (4) arXiv: `validate.sh` + freeze/sign + reviewer send (4/5 closed), (5) springs: tideGlass cell boot, hotSpring viz, esotericWebb V32.
...
- ~~**riboCipher auto-detect dispatch**: domain-level `ribocipher: bool` in TOML, auto-inherits to translations~~ — **SHIPPED** (`1ff5859c`) — sweetGrass/rhizoCrypt auto-use `send_ribocipher_jsonrpc()`.
...
- ~~**N4**: Provenance Trio routing~~ — **9/11 VERIFIED** (rhizoCrypt PASS, loamSpine PASS. sweetGrass: FAIL in exp121 pre-autodetect, **NOW FIXED** by `1ff5859c`. Needs redeploy to confirm.)
...
- **N-series summary**: exp118 14/14, exp119 12/12, exp120 29/29 (34 caps, 58 methods, 11/11 domains), exp121 32/36. **87/91 total**. 2 remaining failures: sweetGrass (fixed in code, needs redeploy) + toadStool (TARPC protocol mismatch — architecture decision, not P1).
...
- **Neural API evolution**: **N2-N5 VERIFIED** (primalSpring exp118-121: 87/91 total, 1.3ms mean dispatch, 9/11 forward). **Dispatch reorder + riboCipher auto-detect SHIPPED**. Remaining gap: **depot rebuild** (current depot binary is Jul 15, pre-dispatch-fix). Once rebuilt + redeployed, sweetGrass will forward (32/36→36/36). toadStool TARPC mismatch is architecture (not P1). Fleet-wide swarmVine deploy + biomeOS → swarmVine gossip table are Phase 3.
...
*Last used*: Wave 157a N2-N5 verified + riboCipher auto-detect shipped — primalSpring exp118-121: 87/91, 1.3ms dispatch (was 15s). biomeOS riboCipher auto-detect (`1ff5859c`): domain-level TOML flag → sweetGrass/rhizoCrypt auto-route. cellMembrane transport unification (`f5033f2`): `#[cfg(unix)]` 7→3, 1,329 tests. Critical path: depot rebuild (Jul 15 binary) → gate redeploy → fleet-wide activation. 17 COMPLETE, 21 ACTIVE, 26 GLACIAL. 64 goals. 16 primals. ~141K+ tests. (Aug 8, 2026 6:45PM)
*Created*: Wave 139a
*First fossilization*: Wave 150p
*Latest fossilization*: Wave 157a N2-N5 verified — primalSpring dispatch verification (87/91). biomeOS riboCipher auto-detect shipped. cellMembrane transport unification. G34 reclassified GLACIAL. Critical path: depot rebuild → gate redeploy → fleet-wide activation. (151+ total across 12 checkpoints, 1,472+ total records)
