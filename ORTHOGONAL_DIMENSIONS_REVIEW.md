...
- [x] bearDog: **14,019** tests. **P0-A CODE FIXED** (`766951004`): health socket guard — rejects non-health methods with -32601. Socket renamed `beardog-default→beardog-health`. No more silent swallowing. Self-audit: all 15 handler kinds verified. **Depot rebuild needed** to deploy fix. (**Wave 157a vertebrate self-audit**)
...
- [x] biomeOS: **8,700+** tests (578 Neural API), **v4.57+**: G67 forwarding fix + Stage 2 routing infra. riboCipher dual-lane pool, Bootstrap→Coordinated watcher, TOML capability translations. **P0-C FIXED** (`6a51638d`): recursive self-referential socket amplification eliminated — L5 perceptron `shadow_compare_remote` was pointing to Neural API's own socket (3^N recursion). Fix: remove self-ref, add re-entrancy guard, remove per-dispatch health storm, `capability.call` now opens at most 1 pooled connection per forward. **Depot rebuild needed** to deploy. (**Wave 157b**)
...
| biomeOS | **20.4 MB** | 377 | 26 | 302K | **P0-C FIXED** (`6a51638d`). Recursive self-ref dispatch eliminated. `capability.call` now 1 pooled conn per forward. Depot rebuild needed. |
| toadStool | **12.4 MB** | 627 | 14 | **708K** | **`core` 272K → natural S371 WASM split.** 24/48 done. |
| bearDog | **8.3 MB** | 556 | **31** | 498K | ~~P0-A~~ CODE FIXED (`766951004` health guard). Depot rebuild needed. |
...
- [x] **bearDog**: ~~P0-A~~ CODE FIXED — health guard, -32601 for non-health. Depot rebuild pending.
- [x] **nestGate**: ~~P0-B~~ RESOLVED — `content.ingest` shipped S136, `content.stat` shipped. Stale depot was root cause.
- [x] **biomeOS**: ~~P0-C~~ FIXED (`6a51638d`) — recursive self-ref dispatch eliminated. At most 1 pooled connection per forward. Depot rebuild needed.
...
| G56/G67 | **Neural API activation & Stage 2 routing** | **ACTIVE — PHASE 3 CODE-COMPLETE, FD LEAK FIXED** | N-series 90/91. Phase 3 gossip integration shipped (`993b97f7`). Vine-bat operational (`df97b25`). `capability.resolve` works (7ms). **P0-C FIXED** (`6a51638d`): recursive self-ref dispatch eliminated, re-entrancy guard added, per-dispatch health storm removed. `capability.call` now opens at most 1 pooled connection per forward. **Both resolve and call paths operational.** Depot rebuild needed to deploy fix fleet-wide. *(Merged G56+G67.)* |
...
**Summary**: Wave 157c ZERO P0 + MESH-NATIVE BUILD. All 3 P0s resolved (biomeOS FD `6a51638d`, bearDog health `766951004`, nestGate stale depot). blueGate primary build authority — 14/14 vertebrate built (23 min), `builder.serve :9800` mesh-native dispatch (Tower Atomic, no SSH). Build authorities: `[blueGate, sporeGate, eastGate]`. Depot vertebrate live (8 rebuilt, golgi 19/19, 13,910 caps). strandGate: AMD DF64 > NVIDIA (24.1 vs 18.1 TFLOPS), NPU <2W free silicon, 75/87 thermalization. **16 primals. 0 P0. ~142K+ tests.**

**Phase shift**: **"Vertebrate complete. Build mesh-native. Neural API call path unblocked. Entering primal evolution phase."** Deploy infrastructure fully solved — mesh-native builds, depot integrity, cascade automation, zero drift. Infrastructure work now yields to primal evolution: songBird transport convergence, petalTongue doom-core extraction, toadStool WASM split, swarmVine data/compute gossip integration, spring cell boots. The next wave of work is domain-focused, not infrastructure-focused. primalSpring owns eastGate hardware cascade. Overwatch reviews temporally through wateringHole.
...
- strandGate: **COMPUTE + SILICON.** **13/13 ALIVE** (swarmVine deployed, dispatch 16ms, 5 mesh peers). **AMD DF64 > NVIDIA DF64** (24.1 vs 18.1 TFLOPS, AMD 1:16 vs NVIDIA 1:64 FP64 ratio). **AMD ROP atomics 7.4x faster** (117.7 vs 16.0 Gatom/s — scatter-add force accumulation). **NPU <2W free silicon** (AKD1000 VFIO-bound, real-time phase monitoring). GPU Lanczos at machine epsilon (8.88e-16). 75/87 thermalization cached (SU(2)+SU(3) complete, SU(4) in progress).
- **sporeGate: TOPOLOGY OWNER + FALLBACK BUILDER.** **DEPOT VERTEBRATE Aug 9** (8 primals rebuilt from vertebrate HEAD). **15/15 ALIVE**, 13,910 caps. **Wave 157c: Delegated build authority to blueGate** — `dispatch_to_sub_builder` uses `MeshRelay{blueGate, build}` via songBird mesh (no SSH). Falls back to local build if mesh fails. Build authorities: `[blueGate, sporeGate, eastGate]`. Vine-bat operational. songBird mesh 11 peers. FD exhaustion fixed (LimitNOFILE=65536). nestgate.io 10/12. **biomeOS P0-C fix (`6a51638d`) needs next depot rebuild.**
...
*Last used*: Wave 157c ZERO P0 + MESH-NATIVE BUILD. All 3 P0s resolved. blueGate primary builder (14/14 vertebrate, 23 min, Tower Atomic mesh dispatch, no SSH). Build authorities: [blueGate, sporeGate, eastGate]. strandGate AMD DF64 > NVIDIA, NPU free silicon. Depot vertebrate live (13,910 caps). Infrastructure solved — entering primal evolution phase. 10 ACTIVE dimensions, 14 FOSSILIZED. 16 primals. 0 P0. ~142K+ tests. (Aug 9, 2026 12:31PM)
*Created*: Wave 139a
