...
## Update: CTXNOTVALID Root Cause (March 12, later)

### Critical Discovery

CTXNOTVALID occurs on **BOTH GPUs**, not just Volta:
- RTX 3090 (GA102, SM86, 0000:21:00.0) — kernel has GSP firmware
- Titan V (GV100, SM70, 0000:4b:00.0) — no GSP

This proves CTXNOTVALID is a **channel setup issue in nouveau**, not
a missing FECS firmware problem. Kernel 6.17's nouveau CHANNEL_ALLOC
creates bare channels without binding the compute engine (PGRAPH) context.

### Firmware Analysis

`sw_method_init.bin` for GV100:
- 1537 entries, ALL with addresses 0x0080902D–0x0DFFC397
- Zero entries are valid push buffer methods (13-bit max: 0x7FFC)
- These are BAR0 register addresses (GPC/TPC/SM space)
- They CANNOT be submitted via channel push buffers

Previous attempts to submit these as push buffer method writes caused
garbage headers (truncated 32-bit addresses to 13-bit method fields)
which killed channels with ENODEV.

### What Fixes Are Needed

The compute engine PGRAPH context must be initialized before SEND_PCAS.
NVK (Mesa's Vulkan driver for nouveau) handles this via:

1. **GR class binding** — NVK binds the GR/compute class to a subchannel
   using specific PGRAPH init methods during channel setup
2. **Context creation** — NVK creates a GR context object and attaches
   it to the channel before dispatch
3. **PGRAPH init sequence** — method writes to the GR class (not raw
   register writes) that configure the compute pipeline

### Recommended Investigation

1. Trace NVK's channel/compute setup via `strace` to see the ioctl
   sequence between CHANNEL_ALLOC and first compute dispatch
2. Compare with our CHANNEL_ALLOC → EXEC sequence — what's missing?
3. Check if nouveau exposes a GR context creation ioctl or if NVK
   uses a different channel creation path

### Kernel Logs (kernel 6.17.9-76061709-generic)

```
nouveau 0000:21:00.0: fifo:PBDMA0: CTXNOTVALID chid:4
nouveau 0000:21:00.0: fifo:c00000:0004:0004:[...] errored - disabling channel
nouveau 0000:4b:00.0: fifo:PBDMA0: CTXNOTVALID chid:2
nouveau 0000:4b:00.0: fifo:000000:0002:0002:[...] errored - disabling channel
```
