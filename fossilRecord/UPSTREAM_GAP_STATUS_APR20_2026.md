...
| `capability_registry.toml` has no `[translations.tensor]` | biomeOS | barraCuda's 59 JSON-RPC methods (Sprint 51: +btsp.negotiate Phase 3) have no translation entries |
...
|--------|---------|-------|--------|
| barraCuda | Sprint 51 | 4,422+ pass | LIVE — 59 JSON-RPC methods (+1 Sprint 51: btsp.negotiate Phase 3 cipher upgrade), BTSP Phase 3 negotiate handler implemented (HKDF-SHA256 key derivation, transport-layer encrypt switch), discovery self-registration, NUCLEUS env wiring, Phase 56 GAP-11 14/18 resolved |
