...
**Last Updated**: April 28, 2026 (Phase 55c)
...
**Phase**: Foundation  
**Status**: Production Ready (A++ GOLD STANDARD) — S208 (April 28, 2026) — 7,842 lib tests, clippy pedantic clean, all files <800L, 49 unsafe blocks SAFETY-documented, zero `Box<dyn Error>`, zero production `TODO`/`FIXME`. Encrypted compute dispatch (S205: purpose-key retrieval + ChaCha20-Poly1305 encrypt/decrypt on dispatch payloads via BearDog), `DISCOVERY_SOCKET` integration (S205: highest-precedence tier for all capability resolution), self-registration via `register_with_discovery()` (S207: `ipc.register` with `["compute.dispatch","compute.capabilities"]` on both startup paths), lint evolution (S206: all `#[allow]` → `reason=`, workspace dep unification, stale feature removal, mock default policy), deep debt (S208: `expect`→`Result`, port constant extraction). 96+ JSON-RPC methods, ~83% line coverage, BearDog crypto delegation enforced, capability-based discovery, hw-learn pipeline, PcieTopologyGraph, ProviderRegistry
...
**Phase**: Foundation  
**Status**: Production Ready (A+) — v0.3.12 (Sprint 47b) — 272+ non-GPU tests + 29 BTSP tests, 50 IPC methods, 803 WGSL shaders, 1,060+ Rust source files, zero unsafe, zero clippy warnings, AGPL-3.0-only, Discovery self-registration (`register_with_discovery()` at startup), 11 semantic capability tags, role-based internal naming (zero hardcoded sibling primal names), typed `NagaExecError` (no silent fallbacks), 12-axis deep debt audit clean, full NUCLEUS env wiring (BEARDOG_SOCKET + BTSP_PROVIDER_SOCKET + DISCOVERY_SOCKET + FAMILY_SEED + BIOMEOS_SOCKET_DIR), NVVM device poisoning guard, DF64 safety probing, capability-based discovery, 3-tier precision model (F32/F64/Df64), `CompileWgslRequest.fp64_strategy` IPC hint, `bytes::Bytes` zero-copy I/O, `service` subcommand (genomeBin), FMA policy, health module (pkpd, microbiome, biosignal); budded from ToadStool (S93), separate primal at `ecoPrimals/barraCuda/`
...
**Role**: coralReef is the sovereign Rust GPU shader compiler. It compiles WGSL, SPIR-V, and GLSL compute shaders to native GPU binaries with full f64 transcendental support on all NVIDIA generations (SM35–SM120, Kepler through Blackwell). AMD backend operational (RDNA2/GFX1030) with E2E dispatch verified. coralDriver provides userspace GPU dispatch via DRM ioctl (AMD amdgpu, NVIDIA nouveau, nvidia-drm/UVM) and VFIO direct BAR0/DMA dispatch (maximum sovereignty). coralGpu unifies compilation and dispatch into a single API with sovereign driver preference (vfio > nouveau > amdgpu > nvidia-drm). `compile_ir()` accepts pre-built Shader IR for Naga bypass. Zero C dependencies, zero vendor lock-in, zero FFI. Part of the sovereign compute pipeline: barraCuda generates WGSL shaders, toadStool proxies `shader.compile.*` requests, coralReef compiles to native binary, coralDriver dispatches on hardware.
...
| **NVIDIA Backend** | SM35-SM120 (Kepler through Blackwell), SASS binary output, f64 transcendentals via MUFU seed + Newton-Raphson on all gens (sqrt, rcp, exp2, log2, sin, cos), QMD v5.0 for Blackwell |
...
**Phase**: Post-NUCLEUS  
**Status**: Production Ready (v0.7.28, 1,462 tests, 91.7%+ coverage, Wire Standard L3 FULL COMPLIANCE, BTSP Phase 2, Tower signing delegation via `CryptoDelegate` (`braid.create` + `anchoring.anchor` → BearDog `crypto.sign` Ed25519, `Witness::from_tower_ed25519` with `tier: "tower"`, graceful degradation), `hostname` crate eliminated (pure Rust), `BraidSignature` removed (Witness-only), env vars fully centralized, ecoBin compliant, musl-static ready, redb default, parking_lot locks, Edition 2024, MSRV 1.87, AGPL-3.0-or-later, pedantic+nursery clean, zero unsafe, zero production unwrap, zero TODO/FIXME, 32 JSON-RPC methods, sled eliminated, libsqlite3-sys eliminated, async-trait zero, dyn dispatch zero, `detect_protocol` three-way multiplexer)
...
**Phase**: Post-NUCLEUS  
**Status**: Production Ready (v0.9.16 Apr 28, 1,509 tests, 92%+ line / 89%+ region coverage, Tower-signed ledger entries via BearDog `crypto.sign_ed25519` (`entry.append` + `session.commit` carry `tower_signature` metadata), `prepare_entry()` + `append_prepared_entry()` signing split, BTSP Phase 2 handshake (tunnel consumption documented as next frontier), pure Rust, ecoBin compliant, UniBin, Edition 2024, pedantic+nursery clean, cast lint deny, `#[expect(reason)]` bulk migration, tarpc 0.37, `ResilientSyncEngine`, cargo deny 4/4 clean, domain-based socket naming, `--socket`/`--port` CLI flags, SignalHandler, UDS backpressure semaphore, TCP_NODELAY)
...
| Category | Details |
|----------|---------|
| **Experiments** | 75 across 17 tracks: Atomic Composition, Graph Execution, Emergent Systems, Bonding & Plasmodium, Cross-Spring Coordination, Showcase-Mined, Live Composition, Multi-Node Bonding, Cross-Gate Deployment, Composition Parity, Cross-Architecture Deployment. All use real `discover_by_capability()` with honest `check_skip` for live-IPC checks. |
| **Deploy Graphs** | 56 TOMLs (6 fragments + 9 profiles + 5 multi-node + 4 spring validation + 2 spring deploy + 3 downstream + 5 bonding + 2 chaos + 2 cross-spring + 4 patterns + 1 federation + root): fragment-first `resolve = true`, all topologically sorted via `topological_waves()` (Kahn's algorithm). |
| **Cross-Arch Binaries** | 46 binaries across 6 targets (x86_64-unknown-linux-musl, aarch64-unknown-linux-musl, armv7-unknown-linux-musleabihf, x86_64-pc-windows-gnu, aarch64-linux-android, riscv64gc-unknown-linux-musl). `build_ecosystem_genomeBin.sh` 3-tier matrix. |
| **Niche Self-Knowledge** | 47 capabilities, semantic mappings (7 domains), operation dependencies, cost estimates |
...
| **Live Composition** | Tower STABLE (beardog + songbird + biomeOS). Full NUCLEUS 12/12 alive (local + benchScale Docker). Remote fetch sim 13/13 checksums verified. Pixel aarch64 payload staged. |
| **Deployment Validation** | Local x86_64: 12/12 primals. benchScale Docker: 12/12 primals. Remote fetch: 13/13 BLAKE3 match. Pixel: 13 aarch64 binaries validated (ELF 64-bit ARM, static, stripped). |
