...
| ludoSpring | V47 (100 experiments, 791 tests, guideStone readiness 4, guideStone standard v1.2.0, three-tier: 20 bare + 15 IPC + 8 NUCLEUS cross-atomic, BLAKE3 Property 3, upstream `call_or_skip`/`is_skip_error`, v1.2.0 tolerance ordering, genomeBin v5.1, all blockers resolved) |
...
**Status**: v0.9.17 Phase 45 — 75 experiments (17 tracks), 631 tests (585 passed + 46 ignored), 46 cross-arch binaries (6 target triples, Tier 1 39/39), genomeBin v5.1, guideStone Level 4 (67/67 live NUCLEUS, 41/41 bare), 47 capabilities, NUCLEUS VALIDATED (Tower + Nest + Node + full 12-primal deployment validated locally + benchScale Docker + remote fetch sim), `build_ecosystem_genomeBin.sh` (9-target matrix), 56 deploy graphs (fragment-first `resolve = true`), MCP tools (8 typed), zero clippy warnings (pedantic+nursery), zero unsafe, ecoBin compliant, all `primal_names::` constants, centralized `tolerances::`, capability-based discovery (6-tier + Neural API)
