...
**Phase**: Foundation  
**Status**: Production Ready (A++ GOLD STANDARD) — S327+ (June 28, 2026) — 23,000+ workspace tests (9,171+ lib-only), clippy pedantic clean (zero warnings), all tests passing, **112 JSON-RPC methods** (17 capability groups), **~85%+ line coverage** (target 90%), 100% SPDX AGPL-3.0-or-later headers, zero production files >750L, zero hardcoded cross-primal names, zero production mocks/TODO/FIXME, zero production raw env string literals (`socket_env` SSOT, S321), dispatch hot-path zero-copy optimized, workspace dep drift zero (`bytes`/`ruzstd`/`serialport`/`ndarray` unified S321), Duration dedup (`CPU_USAGE_SAMPLE_WINDOW` + 8 named constants, S321), reagent module split (`capture.rs` + `mmiotrace.rs`, S321), client riboCipher mock fix (12 tests unblocked S322), composition tests graduated (+11 S322), ipc_watch coverage (9 tests S322), mmio/trials test extraction (S322), telemetry wire-contract constants (S322), test extraction complete (method_gate 644→279, job 652→277, shader_dispatch 590→471, S323), submit param split (642→487+174L, S323), cpu_resource resilience extraction (675→536+154L, S323), flaky test fix (background monitoring un-ignored, S323), edge communication coverage (+9 tests, S323), test graduation (6 un-ignored, S324), catalyst watchdog coverage (15 tests, S324), dispatcher mock E2E (7 tests, S324), MMIO split (613→275+179+194L, S324), `capability_registry.toml` authoritative. riboCipher: CLEAR (`0xEC`) + MitoBeacon (`0xED`) accepted; REJECT on unsignalled. gRPC + OpenCL **deleted** (S319). NUCLEUS 13/13 on eastGate (user systemd). BearDog crypto delegation enforced (Node Atomic), capability-based discovery, hw-learn pipeline, nvpmu BAR0→RegisterAccess, SPIR-V codegen safety, FirmwareInventory, PrecisionBrain routing, NvkZeroGuard, VRAM-aware workload routing, ProviderRegistry, 6 SpringDomains + HealthSpring, PcieTopologyGraph, TOADSTOOL-AUTO-REGISTER (PCI sysfs GPU/NPU inventory)
...
| **IPC** | 112 JSON-RPC 2.0 methods (17 capability groups), tarpc typed RPC, Unix socket standard |
...
**Phase**: Post-NUCLEUS  
**Status**: Production Ready (A+) — v1.6.6, 18 crates, 360 workspace tests, ~90% coverage, edition 2024, `#![forbid(unsafe_code)]` + `deny(unwrap/expect)`, zero C deps, AGPL-3.0-or-later, 56 IPC methods, 13 DataBinding variants, UUI glossary module, SAME DAVE model, client-side WASM rendering (14 exports), showcase fossilized (Wave 49), gate mesh topology visualization (Wave 116), single AEAD (XChaCha20-Poly1305), feature-gated TUI/UI (Wave 120), legacy renderers removed, GPU compute topology (Wave 124), physical topology API (Wave 128), Flint H1 plasma membrane topology cutover (Wave 128). See [PETALTONGUE_NEEDS_FROM_ECOSYSTEM.md](petaltongue/PETALTONGUE_NEEDS_FROM_ECOSYSTEM.md) for integration gaps
...
**Phase**: Post-NUCLEUS  
**Version**: 0.14.17  
**Status**: Production Ready (1,866 tests, 93.7% line coverage, clippy pedantic+nursery clean, Edition 2024, MSRV 1.87, `unsafe_code = "deny"` / `unwrap_used`+`expect_used = "deny"` workspace-wide, AGPL-3.0-or-later + ORC + CC-BY-SA (scyBorg triple-copyleft), UniBin compliant, cargo-deny enforced, `niche.rs` self-knowledge with MCP tools, `capability_registry.toml` (37 methods, 7 domains) + deploy graph, `DagBackend` enum dispatch (redb default), GC sweeper, connect-probe liveness (v1.1.5), capability domain symlinks (v1.3.0), env-configurable operational tuning, adapter-agnostic runtime messaging, zero cross-primal compile deps — sovereign wire types)
