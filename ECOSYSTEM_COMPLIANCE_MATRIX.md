...
- **BearDog**: **STADIAL GATE CLEARED — 13/13** (Wave 55) + **Deep debt clean** (Wave 56) + **Spring audit resolved** (Wave 58). `BEARDOG_FAMILY_SEED` requirement documented in README (was silent failure). `capabilities.list` now advertises `cleartext_available: true` + `cleartext_methods` array — springs can call `crypto.hash` without BTSP via first-byte `{` bypass. Zero `async-trait`/`tarpc`/`opentelemetry`/`ring`/`sled`/`openssl`/`serde_yaml` in lockfile. 18 enum dispatch types. All production files <800 LOC. 14,786+ tests, **90.51%** coverage. Clippy + rustdoc clean (`-D warnings`). Edition 2024.
